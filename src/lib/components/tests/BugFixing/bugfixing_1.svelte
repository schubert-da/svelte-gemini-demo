...
<div class="flex flex-col gap-1">
	...
	<label class="block text-magnum-900" use:melt={$label}>Favourite marine</label>
	<button
			...
			{#each Object.entries(options) as [key, arr]}
				<div use:melt={$group(key)}>
                    ...
                    data-[disabled]:opacity-50"
							use:melt={$option({ value: group + '-' + item, label: group + '-' + item })}
						>
							<div class="check {$isSelected(group + '-' + item) ? 'block' : 'hidden'}">
								<Check class="size-4" />
							</div>
