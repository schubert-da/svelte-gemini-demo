<script>
	...
	const {
		elements: { root: tagsInputRoot, input: tagsInput, tag, deleteTrigger, edit },
		states: { tags }
	} = createTagsInput({
		defaultTags: $selected?.map((s) => s.value),
		unique: true,
		add(tag) {
	...
	$: selected.update((prev) => {
		return $tags.map((t) => ({ value: t.value, label: t.value }));
	});
...
<div class="flex flex-col gap-1">
	...
	<div class="relative">
		<div use:melt={$tagsInputRoot}>
			...
			{/each}
			<input use:melt={$tagsInput} type="text" />
		</div>
		<input
			use:melt={$input}
